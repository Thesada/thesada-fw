...
//   LoadProhibited at EXCVADDR 0x00000000 when heap is exhausted.
...
static int8_t _tcp_clear_events(void* arg) {
  ...
  if (!e) return ERR_MEM; // malloc failed - out of memory
...
static int8_t _tcp_poll(void* arg, struct tcp_pcb* pcb) {
  ...
  // ets_printf("+P: 0x%08x\n", pcb);
  lwip_event_packet_t* e = (lwip_event_packet_t*)malloc(sizeof(lwip_event_packet_t));
  if (!e) return ERR_OK; // poll is non-critical, skip if OOM
...
static int8_t _tcp_recv(void* arg, struct tcp_pcb* pcb, struct pbuf* pb, int8_t err) {
  lwip_event_packet_t* e = (lwip_event_packet_t*)malloc(sizeof(lwip_event_packet_t));
  if (!e) {
    if (pb) pbuf_free(pb); // must free or lwIP leaks the buffer
    return ERR_MEM;
  }
  e->arg = arg;
  if (pb) {
...
static int8_t _tcp_sent(void* arg, struct tcp_pcb* pcb, uint16_t len) {
  // ets_printf("+S: 0x%08x\n", pcb);
  lwip_event_packet_t* e = (lwip_event_packet_t*)malloc(sizeof(lwip_event_packet_t));
  if (!e) return ERR_OK; // ack lost is tolerable, lwIP will retransmit
...
static void _tcp_error(void* arg, int8_t err) {
  // ets_printf("+E: 0x%08x\n", arg);
  lwip_event_packet_t* e = (lwip_event_packet_t*)malloc(sizeof(lwip_event_packet_t));
  if (!e) return; // nothing we can do without memory
...
int8_t AsyncServer::_accept(tcp_pcb* pcb, int8_t err) {
  if (!pcb) return ERR_ABRT; // null PCB - lwIP error condition
  // ets_printf("+A: 0x%08x\n", pcb);
  if (_connect_cb) {
...
int8_t AsyncServer::_s_accept(void* arg, tcp_pcb* pcb, int8_t err) {
  if (!arg) return ERR_ABRT; // null arg - server freed before callback
